...
<div class="flex flex-col gap-6">
    <header class="flex flex-col gap-2">
        <a href="{{ url('/'.$vendor->slug) }}" class="font-mono text-mono-label uppercase text-warm transition hover:text-ink">
            ← {{ $vendor->name }}
        </a>
        <h1 class="text-hero font-semibold leading-tight text-ink">Pay {{ $vendor->name }}</h1>
        <p class="text-caption text-muted">FeedAI hält Dein Geld nie — alles geht direkt an den Vendor.</p>
    </header>

    @if (empty($methods))
        <div class="rounded-card border border-line bg-card p-6 text-center text-muted">
            Dieser Vendor hat noch keine Zahlungsmethoden eingerichtet.
        </div>
    @else
        {{-- Tabs --}}
        ...
        @if ($activeTab === 'card' && $vendor->acceptsCards())
            <section class="flex flex-col gap-4">
                <div class="rounded-card border border-line bg-card p-5">
                    <p class="font-mono text-mono-label uppercase text-muted">Fester Betrag</p>
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach ([10000, 25000, 50000, 100000] as $cents)
                            <button type="button"
                                    wire:click="payFixed({{ $cents }}, '{{ __('Quick pay') }}')"
                                    class="rounded-button border border-line bg-canvas px-4 py-2 text-section text-ink transition hover:border-ink/40">
                ...
                <div class="rounded-card border border-line bg-card p-5">
                    <p class="font-mono text-mono-label uppercase text-muted">Freier Betrag</p>
        ...
        @if ($activeTab === 'promptpay' && $vendor->acceptsPromptPay())
            <section class="flex flex-col items-center gap-4 rounded-card border border-line bg-card p-6">
                <p class="font-mono text-mono-label uppercase text-muted">PromptPay QR</p>
                <div class="rounded-image bg-canvas p-3" data-test="promptpay-qr">
                    {!! $this->promptpayQrSvg !!}
                </div>
                <p class="text-caption text-muted">
                    Mit Deiner Thai-Banking-App scannen. Geld geht direkt an den Vendor — FeedAI sieht die Transaktion nie.
                </p>
                <details class="w-full text-caption text-muted">
                    <summary class="cursor-pointer">EMV Payload anzeigen</summary>
        ...
        @if ($activeTab === 'crypto' && $vendor->acceptsStablecoin())
            <section class="flex flex-col gap-4 rounded-card border border-line bg-card p-6"
                    x-data="{
                        copied: false,
                        copy() {
                ...
                <div class="flex items-center justify-between gap-3">
                    <p class="font-mono text-mono-label uppercase text-muted">Wallet-Adresse</p>
                    <flux:badge color="zinc" size="sm" inset="top bottom">
                        {{ $chainLabel[$vendor->stablecoin_chain] ?? $vendor->stablecoin_chain }}
                    </flux:badge>
                </div>

                <code class="break-all rounded-input border border-line bg-canvas px-3 py-3 font-mono text-[12px] text-ink"
